...
export default function ProfessionalAssinatura() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      {statusMessage && (
        <div className={`p-9 rounded-xl border flex items-center gap-8 animate-in fade-in slide-in-from-top-4 duration-300 ${
            ...
            return (
              <div key={pkg.id} className={`bg-[#1C3454] border ${isPopular ? 'border-blue-500/50 scale-105 shadow-[0_0_30px_-5px_rgba(59,130,246,0.3)] z-10' : 'border-[#1C3050]'} rounded-2xl p-8 relative flex flex-col`}>
                ...
                <button
                  ...
                  className={`w-full py-9 ${isPopular ? 'bg-blue-600 hover:bg-blue-500 shadow-xl shadow-blue-600/20' : 'bg-white/5 hover:bg-white/10'} text-white font-bold rounded-xl transition-all border border-[#243F6A] disabled:opacity-50`}
                >
          ...
          {SUBSCRIPTION_PLANS.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`bg-[#1C3454] border ${borderClass} rounded-2xl p-8 relative flex flex-col ${plan.popular ? 'transform scale-105 z-10 shadow-[0_0_50px_-10px_rgba(16,185,129,0.3)]' : ''}`}>
                ...
                <div className="mb-11">
                  <div className={`inline-block px-8 py-6 text-[10px] font-black uppercase tracking-widest rounded-full mb-8 ${
                  ...
                  <p className={`text-xs font-bold mt-6 ${discountColor}`}>{plan.discount} desconto em todas as moedas</p>
                ...
                <button
                  ...
                  className={`w-full py-9 ${btnClass} font-black rounded-xl transition-all disabled:opacity-50 mb-11`}
                >
                  {buyingId === plan.id ? (
                ...
                <div className={`border rounded-xl p-8 ${savingsClass}`}>
      ...
      <div className="grid lg:grid-cols-2 gap-11 pt-6 border-t border-slate-800">
        ...
        <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl overflow-hidden flex flex-col">
          ...
          <div className="bg-gradient-to-r from-emerald-900/30 via-emerald-900/10 to-transparent border-b border-[#1C3050] px-5 py-9 flex items-center justify-between gap-8">
            ...
            {currentSubscription && (
              <span className={`px-2.5 py-6 text-[10px] font-bold uppercase tracking-widest rounded-lg border shrink-0 ${(STATUS_LABELS[currentSubscription.status] ?? STATUS_LABELS['active']).colorClass}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-9">
          <div className="bg-[#1C3454] border border-[#243F6A] rounded-2xl p-11 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid sm:grid-cols-3 gap-9">
              {SUBSCRIPTION_PLANS.map((plan) => (
                <div key={plan.id} className={`bg-[#0E1C32] border ${plan.popular ? 'border-emerald-500/40' : plan.color === 'blue' ? 'border-blue-500/20' : 'border-yellow-500/20'} rounded-xl p-10 flex flex-col`}>
                  <div className={`inline-block px-7 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-full mb-7 ${
                    plan.popular ? 'bg-emerald-500/20 text-emerald-400' : plan.color === 'blue' ? 'bg-blue-500/20 text-blue-400' : 'bg-yellow-500/20 text-yellow-400'
                  }`}>
                    {plan.discount} OFF
                  </div>
                  ...
                  <button
                    ...
                    } font-bold rounded-xl transition-all text-sm disabled:opacity-50 flex items-center justify-center gap-7`}
                  >
