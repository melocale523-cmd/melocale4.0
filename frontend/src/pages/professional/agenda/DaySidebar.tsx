...
export function DaySidebar({
  ...
  return (
    <div className="bg-[#1C3454] border border-[#1C3050] rounded-3xl p-8 flex flex-col relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-13 relative z-10">
      ...
      <div className="flex-1 space-y-9 relative z-10">
        {selectedDayAppointments.length > 0 ? (
          selectedDayAppointments.map(app => {
            ...
            return (
              <div key={app.id} className="bg-[#0E1C32] border border-[#1C3050] p-10 rounded-2xl space-y-8 hover:border-emerald-500/30 transition-all">
                {clientRequestedReschedule && app.proposed_at && (
                  <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-8">
                    ...
                    <p className="text-xs text-orange-300 mb-8">
                    ...
                    <div className="flex gap-7">
                      <button
                        ...
                        disabled={anyPending}
                        className="flex-1 flex items-center justify-center gap-6 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-black text-[10px] font-bold rounded-lg transition-all disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        disabled={anyPending}
                        className="flex-1 flex items-center justify-center gap-6 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[10px] font-bold rounded-lg border border-red-500/20 transition-all disabled:opacity-50"
                      >
                        <X size={10} /> Recusar
                      </button>
                    </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-7 px-7 py-6 bg-emerald-500/10 rounded-lg text-emerald-500 text-[10px] font-bold font-mono">
                ...
                <div>
                  ...
                  <p className="text-[#4A6580] text-sm flex items-center gap-7 mt-6">
                    <User size={14} /> {app.client?.full_name || 'Cliente'}
                  </p>
                  {app.location && (
                    <p className="text-[#4A6580] text-xs flex items-center gap-6 mt-6">
                      <MapPin size={12} /> {app.location}
                    </p>
                  )}
                  {app.status === 'completed' && (
                    <p className="text-emerald-500 text-[10px] font-bold uppercase tracking-widest mt-7 flex items-center gap-6">
                  ...
                  {isActive(app.status) && (
                    <>
                      <button
                        ...
                        className="p-7 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-lg border border-blue-500/20 transition-all"
                      ...
                      <button
                        ...
                        className="p-7 bg-emerald-500/10 hover:bg-emerald-500 hover:text-black text-emerald-500 rounded-lg border border-emerald-500/20 transition-all"
                        title="Concluir"
                      >
          ...
          <div className="flex flex-col items-center justify-center text-center py-12">
            ...
            <p className="text-white/90 font-bold text-lg mb-7">Nenhum agendamento</p>
            <p className="text-[#4A6580] text-sm">Este dia está livre.</p>
            <button
              onClick={() => onOpenNewForDate(selectedDate)}
              className="mt-10 px-8 py-8 bg-emerald-500 hover:bg-emerald-400 text-black font-bold rounded-2xl transition-all shadow-lg shadow-emerald-500/10 active:scale-[0.98]"
            >
              Agendar Horário
            </button>
          </div>
      ...
      <div className="mt-13 pt-8 border-t border-[#1C3050] relative z-10">
        <div className="flex justify-between items-center text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-9">
          <span>Horários Populares</span>
          <span className="text-emerald-500/50">Disponível</span>
        </div>
        <div className="grid grid-cols-2 gap-7">
          <div className="bg-[#0E1C32] border border-[#1C3050] p-8 rounded-xl text-center text-xs text-slate-300 font-mono italic">09:00 - 10:00</div>
          <div className="bg-[#0E1C32] border border-[#1C3050] p-8 rounded-xl text-center text-xs text-slate-300 font-mono italic">14:00 - 15:00</div>
        </div>
      </div>
    </div>
  );
