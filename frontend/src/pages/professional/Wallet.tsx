...
export default function ProfessionalWallet() {
  ...
  return (
    <div className="space-y-11 max-w-5xl mx-auto">
      ...
      <div className="grid gap-11 md:grid-cols-3">
        ...
        <div className="md:col-span-1 bg-gradient-to-br from-emerald-600/30 via-[#1C3454] to-[#0E1C32] border border-emerald-500/30 rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between shadow-2xl ring-1 ring-emerald-500/20">
          ...
          <div className="relative z-10">
            <div className="flex items-center text-emerald-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-9 bg-emerald-500/10 w-fit px-8 py-6 rounded-full border border-emerald-500/20">
            ...
            {isBalanceLoading ? (
              ...
              <div className="flex flex-col">
                <h2 className="text-6xl font-black text-white tracking-tighter flex items-baseline gap-7">
          ...
          <button 
            ...
            className="mt-10 w-full bg-emerald-500 hover:bg-emerald-400 text-[#0E1C32] py-9 rounded-2xl text-sm font-black uppercase tracking-widest transition-all flex justify-center items-center shadow-xl shadow-emerald-500/20 group/btn active:scale-95">
            <Plus size={20} className="mr-2 group-hover/btn:rotate-90 transition-transform" /> Adicionar Saldo
          </button>
        </div>

        {/* Quick Stats */}
        <div className="md:col-span-2 grid gap-9 sm:grid-cols-2">
           <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl p-11 relative overflow-hidden group">
             <div className="w-10 h-10 bg-slate-800/80 rounded-xl flex items-center justify-center mb-9 text-emerald-500 border border-[#1C3050] group-hover:border-emerald-500/30 transition-colors">
             ...
             <p className="text-xs text-[#4A6580] mt-6 italic">≈ R$ {(((stats?.totalSpentCoins ?? 0) / 10)).toFixed(2).replace('.', ',')}</p>
           </div>
           <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl p-11 relative overflow-hidden group">
             <div className="w-10 h-10 bg-slate-800/80 rounded-xl flex items-center justify-center mb-9 text-blue-400 border border-[#1C3050] group-hover:border-blue-500/30 transition-colors">
               <Receipt size={20} />
             </div>
      ...
      <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl overflow-hidden shadow-xl">
        <ul className="divide-y divide-slate-800/50">
          {transactions && transactions.length > 0 ? (
            transactions.map((tx) => (
              <li key={tx.id} className="p-9 sm:p-5 flex items-center justify-between hover:bg-slate-800/20 transition-colors">
