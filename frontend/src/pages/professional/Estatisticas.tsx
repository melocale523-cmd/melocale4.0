...
export default function ProfessionalEstatisticas() {
  ...
  if (isError || loadingTimedOut) {
    return (
      <div className="max-w-6xl mx-auto flex flex-col items-center justify-center py-24 gap-9">
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-11">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-9">
        ...
        <div className="flex items-center bg-[#1C3454] p-6 rounded-xl border border-[#1C3050]">
          {([
            { id: '7d', label: '7 Dias' },
            ...
            <button
              ...
              className={cn(
                "px-9 py-7 text-xs font-black uppercase tracking-widest transition-all rounded-lg",
                range === item.id
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-9">
        <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl p-11 relative overflow-hidden transition-all hover:border-blue-500/30 group">
          <div className="w-10 h-10 bg-blue-500/10 rounded-xl flex items-center justify-center text-blue-400 mb-9 group-hover:scale-110 transition-transform">
            <Eye size={20} />
          </div>
          <h4 className="text-[#94A3B8] text-[10px] font-black uppercase tracking-widest mb-6">Visualizações</h4>
        ...
        <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl p-11 relative overflow-hidden transition-all hover:border-purple-500/30 group">
          <div className="w-10 h-10 bg-purple-500/10 rounded-xl flex items-center justify-center text-purple-400 mb-9 group-hover:scale-110 transition-transform">
            <TrendingUp size={20} />
          </div>
          <h4 className="text-[#94A3B8] text-[10px] font-black uppercase tracking-widest mb-6">Propostas</h4>
        ...
        <div className="bg-[#1C3454] border border-emerald-500/20 rounded-2xl p-11 relative overflow-hidden shadow-[0_0_15px_-3px_rgba(16,185,129,0.1)] transition-all hover:border-emerald-500/40 group">
          <div className="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-500 mb-9 group-hover:scale-110 transition-transform">
            <CheckCircle2 size={20} />
          </div>
          <h4 className="text-emerald-500 text-[10px] font-black uppercase tracking-widest mb-6">Aceitas</h4>
        ...
        <div className="bg-[#1C3454] border border-[#1C3050] rounded-2xl p-11 relative overflow-hidden transition-all hover:border-yellow-500/30 group">
          <div className="w-10 h-10 bg-yellow-500/10 rounded-xl flex items-center justify-center text-yellow-500 mb-9 group-hover:scale-110 transition-transform">
            <DollarSign size={20} />
          </div>
          <h4 className="text-yellow-500 text-[10px] font-black uppercase tracking-widest mb-6">Faturamento</h4>
      ...
      <div ref={chartsRef}>
        {seriesData.length === 0 ? (
          <div className="grid lg:grid-cols-2 gap-11">
            {[0, 1].map((i) => (
              <div key={i} className="bg-[#1C3454] border border-[#1C3050] rounded-3xl p-8 h-[420px] flex flex-col items-center justify-center gap-8">
      ...
      <div className="bg-[#1C3454] border border-[#1C3050] rounded-3xl p-8">
        ...
        <div className="grid sm:grid-cols-3 gap-8">
          <div className="p-11 rounded-2xl bg-white/[0.02] border border-[#1C3050]">
            ...
            <div className="mt-9 h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
              <div
          ...
          <div className="p-11 rounded-2xl bg-white/[0.02] border border-[#1C3050]">
            ...
            <p className="text-slate-600 text-[10px] mt-6 font-bold">Baseado em serviços aceitos</p>
          </div>
          ...
          <div className="p-11 rounded-2xl bg-white/[0.02] border border-[#1C3050]">
            ...
            <p className="text-slate-600 text-[10px] mt-6 font-bold">Retorno vs Investimento em leads</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02] text-[#4A6580] text-[10px] font-black uppercase tracking-[0.2em]">
                <th className="px-8 py-9">{range === '1y' || range === '90d' ? 'Mês' : 'Data'}</th>
                <th className="px-8 py-9">Total Propostas</th>
                <th className="px-8 py-9">Aceitas</th>
                <th className="px-8 py-9 text-emerald-400">Faturamento</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {[...(stats?.seriesData || [])].reverse().map((day, idx) => (
                <tr key={idx} className="hover:bg-white/[0.01] transition-colors group">
                  <td className="px-8 py-9 text-sm font-bold text-slate-300 group-hover:text-white transition-colors">{day.name}</td>
                  <td className="px-8 py-9 text-sm text-[#94A3B8]">{day.total}</td>
                  <td className="px-8 py-9 text-sm text-[#94A3B8]">
                    <span className={cn(day.aceitas > 0 ? "text-emerald-500 font-bold" : "")}>{day.aceitas}</span>
                  </td>
                  <td className="px-8 py-9 text-sm font-black text-slate-100 italic">
