...
import { FileText, Loader2, ArrowRight, CreditCard, Plus, X, MapPin, Tag, Calendar, MoreVertical, Search, Filter, Inbox, User, DollarSign, Clock, CheckCircle, MessageCircle, Send, Pencil, Trash2 } from 'lucide-react';
...
export default function Pedidos() {
  ...
  const { data: rawPedidos, isLoading } = useQuery({
    queryKey: ['pedidos'],
    queryFn: leadService.getMyRequests,
  });

  const pedidos = rawPedidos as PedidoItem[] | undefined;
  ...
  const createRequestMutation = useMutation({
    ...
    onError: (error: Error) => toast.error(`Erro ao criar pedido: ${error.message}`),
  });
  ...
  const respondMutation = useMutation({
    mutationFn: ({ proposalId, purchaseId, status }: { proposalId: string; purchaseId: string; status: string }) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const budget_min = parseFloat(formData.budget_min);
    const budget_max = parseFloat(formData.budget_max);
    if (editingPedido) {
  ...
  const handleDelete = (pedido: PedidoItem) => {
    setContextMenuId(null);
    if (!window.confirm(`Tem certeza que deseja excluir o pedido "${pedido.title}"?`)) return;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        ...
        <button
          onClick={() => { setEditingPedido(null); setFormData(EMPTY_FORM); setIsModalOpen(true); }}
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-white/[0.03]">
            {filteredPedidos.map((pedido) => (
              <div
                key={pedido.id}
                className="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:bg-white/[0.02] transition-all group active:bg-white/[0.04]"
              >
                <div
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={cn(
                        "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-500/5 text-emerald-500 border border-emerald-500/10 shrink-0",
                    ...
                    {pedido.description && (
                      <p className="text-sm text-slate-500 mb-2 line-clamp-2 leading-relaxed">{pedido.description}</p>
                    )}
                    <div className="flex flex-wrap items-center gap-y-1 gap-x-4">
                      ...
                      <div className="flex items-center text-xs text-slate-500 font-bold">
                        <Calendar size={13} className="mr-1 text-slate-600" />
                        {new Date(pedido.created_at).toLocaleDateString('pt-BR')}
                      </div>
                      {(pedido.budget_min != null || pedido.budget_max != null) && (
                        <div className="flex items-center text-xs text-slate-500 font-bold">
                          <DollarSign size={13} className="mr-1 text-slate-600" />
                          R$ {(pedido.budget_min ?? 0).toLocaleString('pt-BR')} – R$ {(pedido.budget_max ?? 0).toLocaleString('pt-BR')}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between lg:justify-end gap-4 border-t lg:border-t-0 border-white/5 pt-6 lg:pt-0">
                  <div className="text-right hidden sm:block" onClick={() => openProposals(pedido)} role="button">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => openProposals(pedido)}
                      className="w-10 h-10 bg-[#0A0B0D] rounded-xl flex items-center justify-center border border-white/5 text-slate-600 group-hover:bg-emerald-500 group-hover:text-black transition-all"
                    >
                      <ArrowRight size={20} />
                    </button>
                    ...
                    <div className="relative z-20">
                      <button
                        onClick={(e) => { e.stopPropagation(); setContextMenuId(contextMenuId === pedido.id ? null : pedido.id); }}
                        className="w-10 h-10 bg-[#0A0B0D] rounded-xl flex items-center justify-center border border-white/5 text-slate-500 hover:text-white hover:border-white/20 transition-all"
                      >
                        <MoreVertical size={18} />
                      </button>
                      {contextMenuId === pedido.id && (
                        <div className="absolute right-0 top-full mt-1 w-40 bg-[#1C1E24] border border-white/10 rounded-xl shadow-xl z-30 overflow-hidden">
                          <button
                            onClick={(e) => { e.stopPropagation(); openEditModal(pedido); }}
              ...
              <div className="p-20 text-center flex flex-col items-center justify-center gap-4 grayscale opacity-40">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm font-medium">
                    {searchTerm ? 'Tente outros termos de busca.' : 'Você ainda não possui solicitações ativas.'}
                  </p>
                </div>
              </div>
            )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => { setIsModalOpen(false); setEditingPedido(null); setFormData(EMPTY_FORM); }}></div>
          <div className="relative bg-[#14161B] border border-white/10 rounded-[2.5rem] p-8 sm:p-10 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <button
              onClick={() => { setIsModalOpen(false); setEditingPedido(null); setFormData(EMPTY_FORM); }}
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h2 className="text-2xl font-black text-white tracking-tight">
                  {editingPedido ? 'Editar Pedido' : 'Nova Solicitação'}
                </h2>
              ...
              <div className="col-span-2 pt-6">
                ...
                >
                  {isMutating
                    ...
                    : <><Plus size={20} /> {editingPedido ? 'Salvar Alterações' : 'Publicar Pedido'}</>
                  }
                </button>
              </div>
            </form>
