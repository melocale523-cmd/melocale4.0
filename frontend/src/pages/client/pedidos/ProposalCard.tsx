    ...
    <div className="bg-[#1C3454] border border-[#1C3050] rounded-[2rem] p-8 hover:border-blue-500/30 transition-all group overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row justify-between gap-8 relative z-10">
        <div className="flex-1 space-y-11">
              ...
              <div className="flex items-center gap-7 flex-wrap">
                ...
                {proposal.profiles?.id && (
                  <button
                    ...
                    className="text-[10px] font-bold text-blue-400 hover:text-blue-300 border border-blue-500/30 hover:border-blue-400/50 px-7 py-0.5 rounded-full transition-all"
                  >
                    Ver perfil
                  </button>
                )}
                <span className={`text-xs font-semibold px-7 py-6 rounded-full ${cfg.className}`}>
                  {cfg.label}
                </span>
              </div>
          ...
          <div className="bg-[#0E1C32] p-11 rounded-2xl border border-[#1C3050]">
            <p className="text-slate-300 text-sm leading-relaxed italic">"{proposal.description}"</p>
          </div>

          <div className="grid grid-cols-2 gap-9">
            <div className="bg-white/5 p-9 rounded-xl border border-[#1C3050]">
              <p className="text-[10px] font-black text-[#4A6580] uppercase tracking-widest mb-6 flex items-center gap-1.5"><DollarSign size={12} /> Valor do Serviço</p>
              <p className="text-xl font-black text-white">R$ {(proposal.price ?? 0).toLocaleString('pt-BR')}</p>
            </div>
            <div className="bg-white/5 p-9 rounded-xl border border-[#1C3050]">
              <p className="text-[10px] font-black text-[#4A6580] uppercase tracking-widest mb-6 flex items-center gap-1.5"><Clock size={12} /> Prazo Estimado</p>
              <p className="text-xl font-black text-white">{proposal.duration}</p>
            </div>
          </div>
        </div>

        <div className="md:w-64 flex flex-col gap-8 justify-center">
          {proposal.status === 'Enviada' || proposal.status === 'Proposta Enviada' ? (
            <>
              <button
                onClick={() => acceptMutation.mutate({ purchaseId: proposal.id })}
                disabled={anyPending}
                className="w-full py-9 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl transition-all shadow-xl shadow-blue-500/20 active:scale-95 flex items-center justify-center gap-7"
              >
              ...
              <button
                ...
                disabled={anyPending}
                className="w-full py-9 bg-slate-800 hover:bg-slate-700 text-slate-300 font-black rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-7"
              >
                {refuseMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : 'Recusar Proposta'}
              </button>
            </>
          ) : proposal.status === 'Recusada' ? (
            <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-11 text-center space-y-8">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#4A6580] font-bold mt-6">Você recusou esta proposta.</p>
              </div>
            </div>
          ) : (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-11 text-center space-y-9">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#4A6580] font-bold mt-6">O profissional já recebeu seus dados de contato.</p>
              </div>
              <button
                onClick={async () => {
                ...
                className="w-full py-8 bg-emerald-600 text-white text-xs font-black rounded-xl hover:bg-emerald-500 transition-all flex items-center justify-center gap-7"
              >
