  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#1C3454] border border-slate-700 rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        ...
        <div className="px-5 -mt-10 pb-4 border-b border-slate-700/50 shrink-0">
          <div className="flex items-end gap-3 mb-3">
            ...
            <div className="pb-1">
              ...
              {prof && !loading && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${prof.is_active !== false ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}>
              ...
              {prof && (
                <div className="flex flex-wrap gap-2 mt-1">
                  {prof.category
                    ? <span className="text-xs text-emerald-400 font-medium flex items-center gap-1"><Briefcase size={12} /> {prof.category}</span>
                    : <span className="text-xs text-slate-600 italic">Categoria não informada</span>
                  }
                  {prof.city
                    ? <span className="text-xs text-[#94A3B8] flex items-center gap-1"><MapPin size={12} /> {prof.city}</span>
                    : <span className="text-xs text-slate-600 italic">Cidade não informada</span>
                  }
                </div>
              )}
            </div>
          ...
          {loading ? (
            ...
            <>
              {prof?.bio ? (
                ...
                <div>
                  <p className="text-xs font-bold text-[#4A6580] uppercase tracking-widest mb-2">Sobre</p>
                  <p className="text-sm text-slate-600 italic">Este profissional ainda não adicionou uma biografia.</p>
                </div>
              )}
              ...
              {reviews.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {reviews.map(r => (
                      <div key={r.id} className="bg-[#0E1C32] rounded-xl p-4 space-y-3">
