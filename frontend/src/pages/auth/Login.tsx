    ...
    try {
      ...
      const derivedCity = [normAddr.city, normAddr.state].filter(Boolean).join(' - ');
        ...
        fetch(`${API_URL}/api/track/registration`, {
          ...
          body: JSON.stringify({ role: selectedRole, user_id: signUpData.user?.id, email: signUpData.user?.email, phone: formData.phone || undefined, name: formData.name || undefined, city: address.city || undefined, state: address.state || undefined, ...getSignupAttribution(), fbp: getCookie('_fbp'), fbc: getCookie('_fbc') }),
        }).catch(() => {});
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexDirection: 'row', marginBottom: '1.5rem', justifyContent: lockedRole ? 'center' : undefined }}>
                ...
                >
                  ...
                  <div className="w-full flex items-center justify-center gap-2 h-12 bg-white hover:bg-slate-100 text-[#0E1C32] rounded-xl font-black text-sm transition-all">
