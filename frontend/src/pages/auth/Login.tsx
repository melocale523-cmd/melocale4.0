      ...
      {!isAuthenticated && selectedRole !== 'admin' && !isLockedMode && (
        <div className="flex bg-[#0E1C32] p-1.5 rounded-2xl mb-13 border border-[#1C3050]">
          <button
            ...
            className={cn("flex-1 py-8 text-sm font-black rounded-xl transition-all", !isSignUp ? "bg-white/5 text-white" : "text-[#7A9EBF] hover:text-slate-300")}
          >
            Entrar
          </button>
          <button
      ...
      {selectedRole !== 'admin' && (
        ...
        <div className="grid grid-cols-2 gap-9 mb-13">
          <button
            ...
            className={cn("flex flex-col items-center gap-7 p-9 rounded-2xl border transition-all", selectedRole === 'client' ? "border-blue-500/50 bg-blue-500/10 text-blue-400" : "border-[#1C3050] text-[#7A9EBF] bg-white/[0.02] hover:bg-white/[0.05]")}
          >
            <UserIcon size={20} />
          ...
          <button
            ...
            className={cn("flex flex-col items-center gap-7 p-9 rounded-2xl border transition-all", selectedRole === 'professional' ? "border-emerald-500/50 bg-emerald-500/10 text-emerald-400" : "border-[#1C3050] text-[#7A9EBF] bg-white/[0.02] hover:bg-white/[0.05]")}
          >
            <Briefcase size={20} />
        ...
        >
          ...
          <form onSubmit={isSignUp && authStep === 'basics' ? handleNextStep : handleSubmit} className="space-y-11">
            ...
            <button 
              ...
              type="submit"
              className={cn(
                "w-full h-16 rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-8 shadow-2xl mt-12 uppercase tracking-widest",
                isSignUp 
