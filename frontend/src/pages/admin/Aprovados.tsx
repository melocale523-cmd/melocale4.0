  ...
  return (
    <div className="space-y-11 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-[#94A3B8] mt-6">Lista de todos os usuários com cadastro ativo na plataforma.</p>
      </div>
           ...
           <table className="w-full text-left">
             <thead>
               <tr className="border-b border-[#1C3050] text-sm text-[#94A3B8] font-medium">
                 ...
                 <th className="p-9 pr-6">Data de Aprovação</th>
               </tr>
             </thead>
             <tbody>
               {filteredUsers.map(u => (
                 <tr key={u.id} className="border-b border-[#1C3050] hover:bg-slate-800/20 transition-colors group">
                   ...
                   <td className="p-9 text-[#94A3B8]">{u.role === 'professional' ? 'Profissional' : u.role === 'admin' ? 'Admin' : 'Cliente'}</td>
                   <td className="p-9 pr-6 text-[#94A3B8]">{new Date(u.updated_at || u.created_at || '').toLocaleDateString('pt-BR')}</td>
                 </tr>
               ))}
               {filteredUsers.length === 0 && (
                 <tr>
                   <td colSpan={4} className="p-8 text-center text-[#4A6580]">Nenhum usuário aprovado.</td>
                 </tr>
               )}
             </tbody>
           </table>
