  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-400 mt-1">Lista de todos os usuários com o perfil de cliente.</p>
      </div>
             ...
             <tbody>
               {filteredUsers.map(u => (
                 <tr key={u.id} className="border-b border-white/5 hover:bg-slate-800/20 transition-colors group">
                   ...
                   <td className="p-4 pr-6 text-slate-400">{new Date(u.created_at).toLocaleDateString('pt-BR')}</td>
                 </tr>
               ))}
               {filteredUsers.length === 0 && (
                 <tr>
                   <td colSpan={4} className="p-8 text-center text-slate-500">Nenhum cliente encontrado.</td>
                 </tr>
               )}
             </tbody>
           </table>
