...
const objectiveLabel: Record<Objective, string> = { reach: 'Alcance', client_leads: 'Clientes', professional_signup: 'Profissionais', trust: 'ConfianÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§a', education: 'EducaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o' };
...
export default function MarketingIA() {
  ...
  const overviewQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return await response.json() as { metrics: { total: number; drafts: number; approved: number; published: number; failed: number; estimated_cost_cents: number; likes: number; comments: number; reach: number; clicks: number; conversions: number; leads: number } };
    },
  });
  ...
  const generateDraft = useMutation({
    mutationFn: async () => {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel gerar o conteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo.');
      ...
      if (draft.item.id && contentQuery.data?.visual_enabled !== false) {
        ...
        if (!imageResponse.ok) throw new Error(imageBody.error ?? 'Copy criada, mas nÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel gerar a arte.');
        return imageBody as { item: ContentItem };
      }
      return draft;
    },
    onSuccess: () => { toast.success('ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo completo pronto para sua aprovaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o.'); invalidate(); },
    onError: (error) => toast.error((error as Error).message),
  ...
  const generateImage = useMutation({
    mutationFn: async (id: string) => {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel gerar a imagem.');
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: 'approved' | 'rejected' }) => {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel atualizar o rascunho.');
      return body as { item: ContentItem };
    },
    onSuccess: (_, variables) => { toast.success(variables.status === 'approved' ? 'ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo aprovado.' : 'ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo rejeitado.'); invalidate(); },
    onError: (error) => toast.error((error as Error).message),
  });
  const approveAndPublish = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!approvalResponse.ok) throw new Error(approvalBody.error ?? 'NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel aprovar o conteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo.');
      const publishResponse = await apiFetch('/api/admin/social-content/' + id + '/publish-instagram', { method: 'POST' });
      const publishBody = await publishResponse.json();
      if (!publishResponse.ok) throw new Error(publishBody.error ?? 'ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo aprovado, mas nÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel publicar.');
      return publishBody as { item: ContentItem; instagram_media_id: string };
    },
    onSuccess: () => { toast.success('ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo aprovado e publicado no Instagram.'); invalidate(); },
    onError: (error) => toast.error((error as Error).message),
  });
  const publishInstagram = useMutation({
    mutationFn: async (id: string) => {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o foi possÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­vel publicar no Instagram.');
      return body as { item: ContentItem; instagram_media_id: string };
    },
    onSuccess: () => { toast.success('ConteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo publicado no Instagram.'); invalidate(); },
    onError: (error) => toast.error((error as Error).message),
  });
  ...
  return <section className="space-y-5">
    <header className="rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-500/15 to-[#132540] p-5">
      <div className="flex items-start gap-3"><div className="rounded-xl bg-violet-500/20 p-2 text-violet-200"><Sparkles size={24} /></div><div><h1 className="text-2xl font-bold text-white">Marketing IA</h1><p className="mt-1 max-w-3xl text-sm text-slate-300">A MeloCalÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© prepara a pauta, a copy e a arte automaticamente. VocÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âª sÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ revisa e aprova antes da publicaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o.</p></div></div>
      <div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white/10 px-3 py-1 text-slate-200">Claude: estratÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©gia</span><span className="rounded-full bg-white/10 px-3 py-1 text-slate-200">Gemini: {contentQuery.data?.visual_enabled ? 'imagem habilitada' : 'imagem aguardando chave'}</span><span className="rounded-full bg-white/10 px-3 py-1 text-slate-200">Pesquisa web: {contentQuery.data?.research_enabled ? 'habilitada' : 'desativada'}</span></div>
    </header>
    ...
    <div className="rounded-2xl border border-[#29415f] bg-[#132540] p-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">{[["Curtidas", metrics?.likes], ["Comentarios", metrics?.comments], ["Alcance", metrics?.reach], ["Cliques", metrics?.clicks], ["Conversoes", metrics?.conversions], ["Leads", metrics?.leads]].map(([label, value]) => <div key={String(label)} className="rounded-lg bg-[#0E1C32] p-3"><div className="text-xs text-slate-400">{label}</div><div className="mt-1 text-lg font-bold text-white">{Number(value ?? 0).toLocaleString("pt-BR")}</div></div>)}</div>
      <p className="mt-3 text-xs text-slate-400">Os indicadores ficam em zero ate a publicacao e a coleta de insights/conversoes da rede social.</p>
    </div>
    ...
    <form onSubmit={(event) => { event.preventDefault(); generateDraft.mutate(); }} className="grid gap-3 rounded-xl border border-[#1C3050] bg-[#132540] p-4 md:grid-cols-2">
      ...
      <label className="text-sm text-slate-300">PÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºblico<select value={form.audience} onChange={(event) => update('audience', event.target.value as Audience)} className="mt-1 w-full rounded-lg border border-[#29415f] bg-[#0E1C32] px-3 py-2 text-white outline-none focus:border-violet-400"><option value="client">Cliente</option><option value="professional">Profissional</option><option value="mixed">Misto</option></select></label>
      ...
      <label className="text-sm text-slate-300">ServiÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§o<input maxLength={100} value={form.service} onChange={(event) => update('service', event.target.value)} placeholder="Ex.: Eletricista" className="mt-1 w-full rounded-lg border border-[#29415f] bg-[#0E1C32] px-3 py-2 text-white outline-none focus:border-violet-400" /></label>
      <div className="md:col-span-2 flex flex-wrap items-center justify-between gap-3 border-t border-[#1C3050] pt-3"><label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={form.research} disabled={!contentQuery.data?.research_enabled} onChange={(event) => update('research', event.target.checked)} /> Usar pesquisa web para encontrar dÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºvidas e referÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âªncias pÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºblicas</label><button disabled={generateDraft.isPending || !form.topic.trim()} className="inline-flex items-center gap-2 rounded-lg bg-violet-500 px-4 py-2 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"><Sparkles size={16} />{generateDraft.isPending ? automationPhase === 'image' ? 'Processando imagemÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦' : 'Gerando conteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂºdoÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦' : 'Criar conteÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºdo completo'}</button></div>
    </form>

    <section className="rounded-xl border border-[#1C3050] bg-[#132540] p-4"><div className="flex flex-wrap items-center gap-3"><div className="flex items-center gap-2 text-sm font-semibold text-white"><Eye size={16} /> Filtrar pautas</div><select value={campaignFilter} onChange={(event) => setCampaignFilter(event.target.value)} className="rounded-lg border border-[#29415f] bg-[#0E1C32] px-3 py-2 text-sm text-white"><option value="all">Todas as campanhas</option>{(campaignsQuery.data?.campaigns ?? []).map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-lg border border-[#29415f] bg-[#0E1C32] px-3 py-2 text-sm text-white"><option value="all">Todos os estados</option><option value="pending">Planejadas</option><option value="ready">Prontas para aprovar</option><option value="published">Publicadas</option><option value="failed">Com falha</option></select><span className="text-xs text-slate-400">{visibleItems.length} pauta(s) visiveis</span></div></section>
    {contentQuery.isLoading && <p className="text-slate-400">Carregando rascunhosÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</p>}
    ...
    <div className="grid gap-4 xl:grid-cols-2">{visibleItems.map((item) => <article key={item.id} className="relative overflow-hidden rounded-xl border border-[#1C3050] bg-[#132540]">
      ...
      {generatingImageId === item.id && <div className="flex items-center gap-3 border-b border-violet-400/20 bg-violet-500/10 px-4 py-3 text-violet-100"><LoaderCircle size={18} className="animate-spin" /><div><p className="text-sm font-semibold">Processando imagemÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</p><p className="text-xs text-violet-200/80">A MeloCalÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© estÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ criando uma arte realista e alinhada ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â  marca. Isso pode levar atÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© 90 segundos.</p></div></div>}
