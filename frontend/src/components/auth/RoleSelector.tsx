...
export function RoleSelector({ mode, onSelect, onGoogleLogin }: RoleSelectorProps) {
  ...
  return (
    <div className="text-center">
      <div className="inline-flex items-center gap-7 px-9 py-1.5 rounded-full bg-emerald-500/5 border border-emerald-500/10 mb-13">
        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
        <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Área de Acesso Seguro</span>
      </div>

      <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-9">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-11">
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform mb-13">
            <User size={32} />
          </div>
          <h3 className="text-2xl font-black text-white group-hover:text-blue-400 transition-colors mb-9">
            {mode === 'signup' ? 'Sou Cliente' : 'Área do Cliente'}
          </h3>
          <p className="text-[#7A9EBF] text-sm leading-relaxed mb-11">
            {mode === 'signup'
              ? 'Preciso contratar profissionais para serviços em minha casa'
              : 'Gerencie seus pedidos e contrate profissionais'}
          </p>
          {mode === 'signup' && (
            <ul className="space-y-8 mb-13">
              {['Solicite orçamentos grátis', 'Compare profissionais', 'Contrate com segurança'].map((item, i) => (
                <li key={i} className="flex items-center gap-8 text-xs font-bold text-[#B0C4D8]">
          ...
          <div className="mt-auto flex items-center gap-7 text-sm font-black text-[#7A9EBF] group-hover:text-blue-400 transition-all">
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 group-hover:scale-110 transition-transform mb-13">
            <Briefcase size={32} />
          </div>
          <h3 className="text-2xl font-black text-white group-hover:text-emerald-400 transition-colors mb-9">
          ...
          <div className="mt-auto flex items-center gap-7 text-sm font-black text-[#7A9EBF] group-hover:text-emerald-400 transition-all">
      ...
      <div className="mt-12 pt-8 border-t border-[#1C3050]">
        <p className="text-[#7A9EBF] text-[10px] font-black uppercase tracking-widest mb-11">Ou conecte-se instantaneamente</p>
        <button
          onClick={onGoogleLogin}
          className="w-full h-16 bg-white hover:bg-slate-100 text-black rounded-2xl font-black transition-all flex items-center justify-center gap-8 shadow-2xl group active:scale-95"
        >
        ...
        <button
          ...
          className="mt-13 text-[#7A9EBF] text-sm font-medium hover:text-white transition-colors"
        >
