...
export default function CategoryGrid({ userCity }: Props) {
  return (
    <section className="py-8 md:py-20 bg-[#0E1C32] border-t border-slate-800/50">
      <div className="max-w-6xl mx-auto px-9 sm:px-6 lg:px-8">

        <div className="text-center mb-15">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-13">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 sm:gap-4 mb-15">
          {categories.map(cat => (
            <Link
              key={cat.slug}
              to={`/login?mode=signup&servico=${cat.slug}`}
              className="group flex flex-col items-center gap-8 bg-[#1C3454] hover:bg-emerald-500/10 border border-slate-800 hover:border-emerald-500/40 rounded-2xl p-10 transition-all duration-200"
            >
        ...
        <div className="text-center">
          ...
          <Link
            ...
            className="inline-flex items-center gap-7 bg-emerald-500 hover:bg-emerald-400 text-black font-black px-8 py-8 rounded-xl text-sm shadow-lg shadow-emerald-500/20 transition-all uppercase tracking-wide"
          >
