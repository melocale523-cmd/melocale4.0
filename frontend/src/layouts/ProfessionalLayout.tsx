    ...
    <div className="flex h-screen dark:bg-[#0E1C32] text-slate-900 dark:text-white">
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <aside className="w-48 bg-white/[0.15] backdrop-blur-xl dark:bg-[#132540] dark:backdrop-blur-none border-r border-white/20 dark:border-[#1C3050] relative flex flex-col pt-2">
            ...
            <nav className="flex-1 px-3 mt-2 space-y-0.5 overflow-y-auto pb-2">
              {navItems.map((item) => {
                const isActive = location.pathname.startsWith(item.path);
                return (
                  <Link
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-12 border-b border-white/20 dark:border-[#1C3050] bg-white/10 backdrop-blur-xl dark:bg-[#132540] dark:backdrop-blur-none flex items-center justify-between px-3 sm:px-4 z-10 w-full shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative border-l border-white/30 dark:border-[#243F6A] pl-4" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-[#132540] border border-slate-200 dark:border-[#243F6A] rounded-lg shadow-xl z-50 py-1 overflow-hidden">
                  {/* Profile header */}
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white transition-colors text-left"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-1.5 px-3 py-1.5 text-sm text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors text-left"
                  >
        ...
        <div className="flex-1 overflow-auto">
          <div className="max-w-4xl w-full mx-auto px-6 py-3">
            <Outlet />
          </div>
        </div>
      </main>
