      ...
      <aside className="w-64 border-r border-white/20 dark:border-[#1C3050] bg-white/[0.15] backdrop-blur-xl dark:bg-[#132540] dark:backdrop-blur-none flex flex-col hidden md:flex">
        ...
        <div className="p-4 border-t border-white/20 dark:border-slate-800/50">
          <button
            ...
            className="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white/70 dark:text-[#94A3B8] hover:text-emerald-400 hover:bg-emerald-500/10 transition-all duration-200"
          >
            <LogOut size={18} />
        ...
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <aside className="w-64 bg-white/[0.15] backdrop-blur-xl dark:bg-[#132540] dark:backdrop-blur-none border-r border-white/20 dark:border-[#1C3050] relative flex flex-col pt-4">
            <div className="p-4 flex justify-between items-center border-b border-white/20 dark:border-[#1C3050]">
              ...
              <button onClick={() => setMobileMenuOpen(false)} className="text-white/70 dark:text-[#94A3B8] p-2">
                <X size={20} />
              </button>
            </div>
            <nav className="flex-1 px-3 mt-4 space-y-1 overflow-y-auto pb-4">
              {navItems.map((item) => {
                const isActive = location.pathname.startsWith(item.path);
                return (
                  <Link
            ...
            <div className="p-4 border-t border-white/20 dark:border-[#1C3050]">
              <button
                onClick={handleLogout}
                className="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white/70 dark:text-[#94A3B8] hover:text-emerald-400 transition-all"
              >
                <LogOut size={18} />
                <span>Sair do perfil</span>
              </button>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 border-b border-white/20 dark:border-[#1C3050] bg-white/10 backdrop-blur-xl dark:bg-[#132540] dark:backdrop-blur-none flex items-center justify-between px-4 sm:px-6 z-10 w-full shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileMenuOpen(true)} className="md:hidden text-white/70 dark:text-[#94A3B8] p-2">
              <Menu size={22} />
            </button>
            <button
              onClick={() => navigate(-1)}
          ...
          <div className="flex items-center gap-3">
            ...
            <ThemeToggle />
            <NotificationBell />
            <div className="relative border-l border-white/30 dark:border-[#243F6A] pl-4" ref={menuRef}>
              <button
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-[#132540] border border-slate-200 dark:border-[#243F6A] rounded-xl shadow-xl z-50 py-2 overflow-hidden">
                  {/* Profile header */}
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white transition-colors text-left"
                  >
